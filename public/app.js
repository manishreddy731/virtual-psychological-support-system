...
const toggleAssessmentDrawer = show => {
  if (!assessmentDrawer) return;
  assessmentDrawer.hidden = !show;
};

const renderSuggestionButtons = options => {
  if (!suggestionActions || !assessmentSuggestion) return;
...
const checkBackend = async () => {
  ...
  try {
    ...
    updateConnectionStatus(response.ok ? 'Connected to wellbeing server.' : 'Server unavailable.');
  } catch {
